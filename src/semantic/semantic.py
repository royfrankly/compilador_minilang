...
class SemanticAnalyzer:
    ...
    def generic_visit(self, nodo):
        raise Exception(f"Error interno: Nodo no soportado {type(nodo).__name__}")
    ...
    def visit_Declaracion(self, nodo):
        try:
            self.symbol_table.declare(nodo.id_variable, nodo.tipo_dato, "Desconocida")
        except Exception as e:
            self.errores.append(str(e))

    def visit_Asignacion(self, nodo):
        try:
            variable_info = self.symbol_table.lookup(nodo.id_variable, "Desconocida")
            ...
            if tipo_expresion and variable_info['tipo'] != tipo_expresion and tipo_expresion != 'ERROR':
                # Permitir asignar INT a REAL (coerción implícita)
                if not (variable_info['tipo'] == 'REAL' and tipo_expresion == 'ENTERO'):
                    self.errores.append(f"Incompatibilidad: variable '{nodo.id_variable}' ({variable_info['tipo']}) no acepta valor ({tipo_expresion}).")
        except Exception as e:
            self.errores.append(str(e))
    ...
    def visit_OperacionBinaria(self, nodo):
        ...
        if nodo.operador in ['+', '-', '*', '/']:
            if tipo_izq not in ['ENTERO', 'REAL'] or tipo_der not in ['ENTERO', 'REAL']:
                 self.errores.append(f"Matemática inválida entre tipos '{tipo_izq}' y '{tipo_der}'.")
                 return 'ERROR'
    ...
    def visit_Numero(self, nodo):
        if isinstance(nodo.valor, float): return 'REAL'
        return 'ENTERO'

    def visit_Identificador(self, nodo):
        try:
            var_info = self.symbol_table.lookup(nodo.nombre, "Desconocida")
    ...
    def visit_CondicionalSi(self, nodo):
        ...
        if tipo_cond != 'BOOLEANO' and tipo_cond != 'ERROR':
            self.errores.append("La condición SI debe ser lógica (BOOLEANO).")
        for instr in nodo.bloque_si: self._visit(instr)
        if nodo.bloque_sino:
            for instr in nodo.bloque_sino: self._visit(instr)

    def visit_CicloMientras(self, nodo):
        tipo_cond = self._visit(nodo.condicion)
        if tipo_cond != 'BOOLEANO' and tipo_cond != 'ERROR':
            self.errores.append("La condición MIENTRAS debe ser lógica (BOOLEANO).")
        for instr in nodo.bloque: self._visit(instr)
    ...
    def visit_Leer(self, nodo):
        try:
            self.symbol_table.lookup(nodo.id_variable, "Desconocida")
        except Exception as e:
            self.errores.append(str(e))